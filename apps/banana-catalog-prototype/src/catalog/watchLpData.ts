...
export function watchLpPlaylistKind(pl: YouTubePlaylistCatalogItem): 'songbook' | 'genre' {
  return (pl.playlist_type || '').trim().toLowerCase() === 'genre' ? 'genre' : 'songbook'
}

export function watchLpPlaylistMetaLine(pl: YouTubePlaylistCatalogItem): string {
  const parts: string[] = []
  if (watchLpPlaylistKind(pl) === 'songbook' && pl.sutra?.trim()) parts.push(pl.sutra.trim())
...
function matchesSutraFilter(pl: YouTubePlaylistCatalogItem, sutra: WatchLpSutraFilter): boolean {
  ...
  return (pl.sutra || '').trim().toUpperCase() === sutra.toUpperCase()
}
